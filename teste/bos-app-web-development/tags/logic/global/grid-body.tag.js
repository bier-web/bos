    ...
    this.on("mount", function () {
        if (typeof opts.collection.events !== "undefined") {
            for (index = 0; index < opts.collection.events.length; index++) {
                ...
                // Eventos só podem ser configurados em collections do bbc, em Views, que são codeResources, não é possível.
                bbc[opts.collection.name].off(element.name);
                bbc[opts.collection.name].on(element.name, function (data) {
        ...
        if (opts.properties.gridType == helpersWebApp.gridType.crud && !opts.refreshData && !opts.collection.noUpdateEvent) {
            bbc[opts.collection.name].off("changed");
            bbc[opts.collection.name].on("changed", function (data) {
