...
var getApiKeys = function (callback) {
    bbc.systemsettings.get({ name: "googleApiKey" }, function (systemSettings, error) {
        if (error) {
            ...
            bosScriptExternalFiles.push(
                helpersWebApp.both().formatString("https://maps.googleapis.com/maps/api/js?key={0}&v=3.exp&sensor=false&libraries=places", systemSettings[0].value)
            );
            return callback();
        }
    });
