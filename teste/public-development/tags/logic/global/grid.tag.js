...
gridTag = function (opts) {
	...
	this.on('mount', function () {
		...
		// Caso o objeto venha vazio, então preenche com vazio para mandar ao dpd;
	...
	_self.getPermissions = function (callback) {
		dpd.permissions.get({ userPermissionCollection: opts.collection.name }, function (permissions, error) {
	...
	_self.getDataThenLoadDashboard = function () {
		_self.getPermissions(function () {
			_self.gridFilterOnLoad = function () {
				...
				app.loading(true);
				dpd[optsCollection.viewName || optsCollection.name].get(_queryOptions, function (data, error) {
					if (error) {
						iziToast.error({
		...
		if (opts.properties.filterColumns != undefined) {
			opts.properties.filterColumns.forEach(function (column) {
				...
				switch (column.type) {
					case 'date':
						if (
							!$(String.format('{0} .grid-filter-content[data-id={1}] .grid-filter-form[data-id={1}] #filter-{2}0-checkbox', _self.gridIdClass, _self.gridId, column.name))[0]
								.checked &&
							!$(String.format('{0} .grid-filter-content[data-id={1}] .grid-filter-form[data-id={1}] #filter-{2}1-checkbox', _self.gridIdClass, _self.gridId, column.name))[0]
								.checked
						) {
							return true;
						} else {
							if (
								$(
									String.format('{0} .grid-filter-content[data-id={1}] .grid-filter-form[data-id={1}] #filter-{2}0-checkbox', _self.gridIdClass, _self.gridId, column.name)
								)[0].checked
							) {
								var valueToFindStartDate = $(String.format('.grid-filter-content[data-id={0}] .grid-filter-form[data-id={0}] #filter-{1}0', _self.gridId, column.name)).val();
							...
							if (
								$(
									String.format('{0} .grid-filter-content[data-id={1}] .grid-filter-form[data-id={1}] #filter-{2}1-checkbox', _self.gridIdClass, _self.gridId, column.name)
								)[0].checked
							) {
								var valueToFindEndDate = $(String.format('.grid-filter-content[data-id={0}] .grid-filter-form[data-id={0}] #filter-{1}1', _self.gridId, column.name)).val();
								valueToFindEndDate =
									valueToFindEndDate == ''
										? moment($(String.format('.grid-filter-content[data-id={0}] .grid-filter-form[data-id={0}] #filter-{1}0', _self.gridId, column.name)).val()).format(
					...
					case 'datetime':
						if (
							...
							if (
								...
								var valueToFindStartDate = $(String.format('.grid-filter-content[data-id={0}] .grid-filter-form[data-id={0}] #filter-{1}0', _self.gridId, column.name)).val();
								valueToFindStartDate =
									valueToFindStartDate == '' ? moment(new Date()).format('YYYY-MM-DDT00:00:00.000') : moment(valueToFindStartDate).format('YYYY-MM-DDTHH:mm:00.000');
							} else {
					...
					case 'text':
						if (
							...
							if (typeof column.searchType === 'undefined') {
								var valueToFind = $(String.format('.grid-filter-content[data-id={0}] .grid-filter-form[data-id={0}] #filter-{1}0', _self.gridId, column.name)).val();
