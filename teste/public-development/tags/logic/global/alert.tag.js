...
alertTag = function (opts) {
	...
	this.on('mount', function () {
		...
		if (opts.action == helpersWebApp.both().actions.add) {
			app.loading(true);
			dpd.alerts.post({}, function (newItem) {
				_self.load(newItem);
				app.loading(false);
			});
		}
	...
	this.load = function (alert) {
		...
		if (typeof alert.startDate !== 'undefined' && alert.startDate !== '') $('.alert-tag .alert-form #alert-start-date').val(new Date(alert.startDate).toISOString().slice(0, 10));
	...
	this.removeItem = function (id) {
		dpd.alerts.del(id, function (result, error) {
			if (error) {
				// caso ocorra algum erro ao salvar, não fecha a tela e exibe os erros;
				app.loading(false);
				iziToast.error({
