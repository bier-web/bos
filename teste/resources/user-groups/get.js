...
try {
    ...
    if (!internal) {
        userGroupsBusiness.canGet(ctx, bbc, me, _userGroup, function (canGet) {
    ...
    if (typeof query.include !== "undefined" && query.include.indexOf("usergroupusers") > -1) {
        bbc.usergroupusers.get({ userGroupId: _userGroup.id, userId: query.userId, $ignoreLimitRecursion: true }, function (userGroupsUsers, error) {
            if (error) {
