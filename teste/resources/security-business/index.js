...
let applyDimensionsToQuery = function (ctx, dpd, loggedUser, collectionSettings, queryOptions, callback) {
	try {
...
let canGetData = function (ctx, dpd, loggedUser, collection, callback) {
	try {
		dpd.systemsettings.get({ name: 'systemViews' }, function (systemSettings) {
			...
			if (typeof loggedUser !== 'undefined' && typeof loggedUser.isRoot != 'undefined' && loggedUser.isRoot) {
				...
				if (ctx.session.data.userGroupPermissions.length == 0) {
					...
					helpers.helperServer.showLog(helpers.helperServer.both.formatString('BierOnStack: Recurso SecurityBusiness :: Evento :: canGetData :: Ação :: End {0}', _hasPermission));
...
var hasPermissionToAction = function (ctx, dpd, loggedUser, object, collection, action, callback) {
	try {
		helpers.helperServer.showLog('BierOnStack: Recurso SecurityBusiness :: Evento :: hasPermissionToAction :: Ação :: Begin');
		let _hasPermission = false;
		if ((typeof loggedUser !== 'undefined' && typeof loggedUser.isRoot != 'undefined' && loggedUser.isRoot) || (typeof object.systemIsPost != 'undefined' && object.systemIsPost)) {
			...
			if (ctx.session.data.userGroupPermissions.length == 0) {
				...
				for (var userGroupPermission in ctx.session.data.userGroupPermissions) {
					...
					if (_hasPermission) {
						_canActionByDimension(ctx, dpd, loggedUser, object, collection, function (canAction) {
					...
					if (
						...
						callback(_hasPermission && (typeof object.systemIsLocked == 'undefined' || (typeof object.systemIsLocked != 'undefined' && object.systemIsLocked == false)));
					}
				}
			}
		}
...
var hasPermissionToActionInCollection = function (ctx, dpd, loggedUser, object, collection, callback) {
	try {
...
var prepareObjectToPost = function (ctx, dpd, loggedUser, object, collection, callback) {
	try {
		...
		if (typeof loggedUser !== 'undefined' && typeof loggedUser.isRoot !== 'undefined' && loggedUser.isRoot) {
			...
			ctx.session.data.userGroupDimensions.forEach((userGroupDimension) => {
				if (userGroupDimension.dimensionCollection === collection) {
					...
					helpers.helperServer.showLog(
						helpers.helperServer.both.formatString('BierOnStack: Recurso SecurityBusiness :: Evento :: prepareObjectToPost :: Ação :: End {0}', JSON.stringify(object))
					);
				}
			});
			callback(object);
		...
		helpers.helperServer.showException(helpers.helperServer.both.formatString('BierOnStack: Recurso SecurityBusiness :: Evento :: prepareObjectToPost :: Exceção :: {0}', error.message));
	}
};

var _canActionByDimension = function (ctx, dpd, loggedUser, object, collection, callback) {
	try {
