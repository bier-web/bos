...
try {
	...
	if (!internal) {
		usersBusiness.canGet(ctx, dpd, me, _user, function (canGet) {
			if (canGet) {
				// helpers.helperServer.showLog("BierOnStack: Recurso Users :: Evento :: get :: Ação :: Begin");
			} else {
	...
	if (typeof query.include !== 'undefined' && query.include.indexOf('usergroupusers') > -1) {
		dpd.usergroupusers.get({ userId: _user.id, userGroupId: query.userGroupId, $ignoreLimitRecursion: true }, function (userGroupUsers, error) {
			if (error) {
