...
var supportRequestsBusiness = {
    canPost: function (ctx, bbc, loggedUser, supportRequest, callback) {
        try {
            securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, supportRequest, "supportrequests", helpers.helperServer.both.actions.add, function (canAdd) {
                if (canAdd) {
                    callback(true);
                } else {
    ...
    onPost: function (ctx, bbc, loggedUser, supportRequest, callback) {
        try {
        } catch (error) {
    ...
    canPut: function (ctx, bbc, loggedUser, supportRequest, callback) {
        try {
            securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, supportRequest, "supportrequests", helpers.helperServer.both.actions.edit, function (canEdit) {
                if (canEdit) {
                    callback(true);
                } else {
                    callback(false, "Usuário não tem permissão");
    ...
    onPut: function (ctx, bbc, loggedUser, supportRequest, callback) {
        try {
            if (supportRequest.systemIsPost) {
                module.exports.createAlertForNewSupportRequest(bbc, loggedUser, supportRequest);
            }

            bbc.supportstatuses.get(supportRequest.supportStatusId, function (supportStatus) {
                ...
                auditBusiness.logAction(
                    ...
                    loggedUser,
                    "supportrequests",
                    "(" + supportRequest.id + ") - " + supportRequest.title,
    ...
    createAlertForNewSupportRequest: function (bbc, loggedUser, supportRequest) {
        try {
            ...
            bbc.supportpriorities.get(supportRequest.supportPriorityId, function (supportPriority) {
                bbc.supportstatuses.get(supportRequest.supportStatusId, function (supportStatus) {
                    ...
                    bbc.systemsettings.get({ name: "supportRequestsUserGroupsAlert" }, function (systemSettings, error) {
                        if (error) {
                                            ...
                                            bbc.alerts.post(
                                                {
                                                    ...
                                                    alert:
                                                        "Chamado: " +
                                                        supportRequest.uniqueId +
                                                        " - Aberto por: " +
                                                        supportRequest.userNameCreated +
                                                        " - " +
                                                        supportStatus.name +
                                                        " - " +
                                                ...
                                                function (alert, error) {
                                                    ...
                                                    helpers.helperServer.showLog(
                                                        "BierOnStack: Recurso SupportRequestsBusiness :: Evento :: createAlertForNewSupportRequest :: Ação :: End alerta de solicitação criado"
                                                    );
                                                }
                                            );
                                        }
            ...
            helpers.helperServer.showException(
                helpers.helperServer.both.formatString("BierOnStack: Recurso SupportRequestsBusiness :: Evento :: createAlertForNewSupportRequest :: Exceção :: {0}", error.message)
            );
        }
    },
    notifyUsersAboutRequestChanged: function (bbc, loggedUser, supportRequest) {
        try {
                                                ...
                                                bbc.alerts.post(
                                                    {
                                                        ...
                                                        alert:
                                                            ...
                                                            supportRequest.uniqueId +
                                                            " - Alterado por: " +
                                                            supportRequest.lastMoveUserName +
                                                            " - " +
                                                            supportStatus.name +
                                                            " - " +
                                                            supportPriority.name +
            ...
            helpers.helperServer.showException(
                helpers.helperServer.both.formatString("BierOnStack: Recurso SupportRequestsBusiness :: Evento :: notifyUsersAboutRequestChanged :: Exceção :: {0}", error.message)
            );
        }
    },
    canDelete: function (ctx, bbc, loggedUser, supportRequest, callback) {
        try {
            securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, supportRequest, "supportrequests", helpers.helperServer.both.actions.remove, function (canRemove) {
                if (canRemove) {
    ...
    onDelete: function (ctx, bbc, loggedUser, supportRequest, callback) {
        try {
            ...
            auditBusiness.logAction(
                ...
                loggedUser,
                "supportrequests",
                "(" + supportRequest.id + ") - " + supportRequest.name,
                ctx.req.headers["x-real-ip"]
            );
        } catch (error) {
    ...
    prepareObjectToPost: function (ctx, bbc, loggedUser, supportRequest, callback) {
        try {
            helpers.helperServer.showLog("BierOnStack: Recurso SupportRequestsBusiness :: Evento :: prepareObjectToPost :: Ação :: Begin");
            securityBusiness.prepareObjectToPost(ctx, bbc, loggedUser, supportRequest, "supportrequests", function (objectToPost) {
                bbc.systemsettings.get({ name: { $in: ["supportRequestStatusDefault", "supportRequestPriorityDefault"] } }, function (systemSettings, error) {
                    if (error) {
                        helpers.helperServer.showException(
                            helpers.helperServer.both.formatString("BierOnStack: Recurso SupportRequestsBusiness :: Evento :: prepareObjectToPost :: Exceção :: {0}", error.message)
                        );
                        callback(supportRequest);
                    } else if (systemSettings.length == 0) {
                        helpers.helperServer.showException(
                            "BierOnStack: Recurso SupportRequestsBusiness :: Evento :: prepareObjectToPost :: Exceção :: Sem status e prioridade padrão!"
                        );
                    }
