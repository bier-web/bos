...
var canGet = function (ctx, bbc, loggedUser, userGroupPermission, callback) {
    try {
        securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, userGroupPermission, "usergroups", helpers.helperServer.both.actions.read, function (canRead) {
            callback(canRead);
        });
    } catch (error) {
        ...
        securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, userGroupPermission, "usergroups", helpers.helperServer.both.actions.edit, function (canEdit) {
            if (canEdit) {
                auditBusiness.logAction(
                    ...
                    loggedUser,
                    "Permissão em Grupo Usuário",
                    "(" + userGroupPermission.userGroupId + ") - Permissão - " + userGroupPermission.name,
                    ctx.req.headers["x-real-ip"]
                );
            }
