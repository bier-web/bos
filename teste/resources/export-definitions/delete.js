try {
    ...
    exportDefinitionsBusiness.canDelete(ctx, bbc, me, exportDefinition, function (canDelete, message) {
        if (canDelete) {
            helpers.log("exportdefinitions -> onDelete -> Chamando exportDefinitionsBusiness -> canDelete ok, chamando onDelete");
            exportDefinitionsBusiness.onDelete(ctx, bbc, me, exportDefinition);
        } else {
