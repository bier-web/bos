...
try {
    ...
    if (!internal) {
        menusBusiness.canGet(ctx, me, _menu, session, function (canGet) {
            helpers.helperServer.showLog(
                helpers.helperServer.both.formatString("BierOnStack: Recurso Menus :: Evento :: get :: Ação :: End usuário não autorizado {0}", _menu.name)
            );
            cancelIf(!canGet);
        });
    }
    ...
    if (typeof query.include !== "undefined" && query.include.indexOf("usergroupmenus") > -1) {
        bbc.usergroupmenus.get({ menuId: _menu.id, userGroupId: query.userGroupId, $ignoreLimitRecursion: true }, function (userGroupMenus, error) {
            if (error) {
