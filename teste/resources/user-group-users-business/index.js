...
var canGet = function (ctx, bbc, loggedUser, userGroupUser, callback) {
    try {
        helpers.helperServer.showLog("BierOnStack: Recurso UserGroupUsersBusinesss :: Evento :: canGet :: Ação :: Begin");
        securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, userGroupUser, "usergroups", helpers.helperServer.both.actions.read, function (canRead) {
...
var canPut = function (ctx, bbc, loggedUser, userGroupUser, callback) {
    try {
        helpers.helperServer.showLog("BierOnStack: Recurso UserGroupUsersBusinesss :: Evento :: canPut :: Ação :: Begin");
        securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, userGroupUser, "usergroups", helpers.helperServer.both.actions.edit, function (canEdit) {
            if (canEdit) {
                ...
                auditBusiness.logAction(
                    ...
                    loggedUser,
                    "Usuário no Grupo ",
                    "(" + userGroupUser.id + ") - Usuário: " + userGroupUser.userId + "  - Grupo: " + userGroupUser.userGroupId,
                    ctx.req.headers["x-real-ip"]
                );
            }
            callback(canEdit);
