try {
    ...
    if (typeof ctx.body.include !== "undefined" && ctx.body.include.indexOf("setReaded") >= 0) {
        bbc.alerts.get(alert.id, function (alertUpdated, error) {
            if (error) {
                console.log(error);
            } else {
    ...
    alertsBusiness.canPut(ctx, bbc, me, alert, function (canPut) {
        if (canPut) {
            if (changed("actorId") || changed("userGroupId") || changed("userId")) {
                ...
                alertsBusiness.registerToUsers(ctx, bbc, me, alert, function (arrayUsersToAlert) {
