...
var canGet = function (ctx, dpd, loggedUser, alert, callback) {
	try {
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, alert, 'alerts', helpers.helperServer.both.actions.read, function (canRead) {
			callback(canRead);
		});
	} catch (error) {
...
var canPost = function (ctx, dpd, loggedUser, alert, callback) {
	try {
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, alert, 'alerts', helpers.helperServer.both.actions.add, function (canAdd) {
			callback(canAdd);
		});
	} catch (error) {
...
var onPost = function (ctx, dpd, loggedUser, alert) {
	try {
		securityBusiness.prepareObjectToPost(ctx, dpd, loggedUser, alert, 'alerts', function (alertUptaded) {
			auditBusiness.logAction(dpd, helpers.helperServer.both.actions.add, loggedUser, 'Alerta', '(' + alert.id + ') - ' + alert.title, ctx.req.headers['x-real-ip']);
...
var onPut = function (ctx, dpd, loggedUser, alert, callback) {
	try {
	} catch (error) {
...
var canPut = function (ctx, dpd, loggedUser, alert, callback) {
	try {
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, alert, 'alerts', helpers.helperServer.both.actions.edit, function (canEdit) {
			if (canEdit) {
				auditBusiness.logAction(dpd, helpers.helperServer.both.actions.edit, loggedUser, 'Alerta ', '(' + alert.id + ') - ' + alert.title, ctx.req.headers['x-real-ip']);
				callback(true);
			} else {
				callback(false);
			}
...
var canDelete = function (ctx, dpd, loggedUser, alert, callback) {
	try {
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, alert, 'alerts', helpers.helperServer.both.actions.remove, function (canDelete) {
			if (canDelete) {
				auditBusiness.logAction(dpd, helpers.helperServer.both.actions.remove, loggedUser, 'Alerta', '(' + alert.id + ') - ' + alert.title, ctx.req.headers['x-real-ip']);
			}
...
var onDelete = function (ctx, dpd, loggedUser, alert) {
	try {
	} catch (error) {
...
var registerToUsers = function (ctx, dpd, loggedUser, alert, callback) {
	try {
		var _arrayUsers = [];
		if (alert.userId) {
			...
			dpd.usergroupusers.get({ userGroupId: alert.userGroupId }, function (userGroupUsers, error) {
				if (error) {
			...
			dpd.actors.get(alert.actorId, function (actor, error) {
				if (error) {
					...
					if (actor.isToAll) {
						dpd.users.get({ isActive: true }, function (users, error) {
							if (error) {
						...
						dpd.actors.get(function (actors, error) {
							if (error) {
								...
								dpd.users.get(_filterOptions, function (users, error) {
									if (error) {
										helpers.notifyException('alertsBusiness -> registerToUsers -> Erro buscando usuários (público alvo) ->');
										helpers.notifyException(error);
										callback(_arrayUsers);
