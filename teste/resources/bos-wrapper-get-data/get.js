/* 
  ...
  O que são views: São coleções complexas, criadas utilizando o objecto event get do bbc, encapsulando aggregates ou qualquer outro tipo de dados e informações mais detalhadas;
  O que são collections: São coleções padrão bbc, coleções muitas vezes CRUD`s que tem volume pequeno de dados e pouca complexidade nas informações retornadas;
*/
...
try {
    ...
    let dataResult = {};
    bosWrapperGetDataBusiness.canGetData(ctx, bbc, me, optsCollection, function (canGetData) {
        if (canGetData) {
            bosWrapperGetDataBusiness.applySecurityRoles(ctx, bbc, me, optsCollection, queryOptions, function (securityQueryOptions) {
                ...
                switch (dataType.id) {
                    case helpers.helperServer.both.dataType.singleCollection.id:
                        bbc[optsCollection.name].get(securityQueryOptions, function (resourceResult, error) {
                            if (error) {
                    ...
                    case helpers.helperServer.both.dataType.singleView.id:
                        bbc[optsCollection.viewName].get(securityQueryOptions, function (resourceResult, error) {
                            if (error) {
                                dataResult.result = helpers.helperServer.both.dataResult.error;
                                dataResult.errorMessage = error;
                            } else {
                    ...
                    case helpers.helperServer.both.dataType.paginatedCollection.id:
                        ...
                        bbc[optsCollection.viewName || optsCollection.name].get(
                            Object.assign(sanitizedQueryOptions.queryOptions, { bosAction: "count" }),
                            function (dataCount, error) {
                                queryOptions =
                                ...
                                bbc[optsCollection.viewName || optsCollection.name].get(securityQueryOptions, function (data, error) {
                                    if (error) {
                                        dataResult.result = helpers.helperServer.both.dataResult.error;
                                        dataResult.errorMessage = error;
                                    } else {
                    ...
                    case helpers.helperServer.both.dataType.paginatedViewCollection.id:
                        ...
                        sanitizedQueryOptions = helpers.helperServer.sanitizePaginnatedOptions(securityQueryOptions);
                        bbc[optsCollection.viewName || optsCollection.name].get(sanitizedQueryOptions.queryOptions, function (resourceResult, error) {
                            if (error) {
                                dataResult.result = helpers.helperServer.both.dataResult.error;
                                dataResult.errorMessage = error;
                            } else {
                    ...
                    case helpers.helperServer.both.dataType.exportData.id:
                        $addCallback();
                        bbc.systemsettings.get({ name: "maxExportsItemsOnGrid", $ignoreLimitRecursion: true }, function (systemSettings, error) {
                            if (error || systemSettings.length == 0) {
                                ...
                                bbc[optsCollection.viewName || optsCollection.name].get(Object.assign(securityQueryOptions, { bosAction: "count" }), function (dataCount, error) {
                                    ...
                                    if (dataCount.count > parseInt(systemSettings[0].value)) {
                                        ...
                                        bbc.exportsmanager.post(
                                            ...
                                            function () {
                                                helpers.helperServer.showLog(
                                                    "BierOnStack: Recurso bosWrapperGetData :: Evento :: get :: Ação :: End exportando relatório em background"
                                                );
                                                setResult(dataResult);
                                                $finishCallback();
                                            }
                                        ...
                                        bbc[optsCollection.viewName || optsCollection.name].get(securityQueryOptions, function (resourceResult, error) {
                                            dataResult.data = Array.isArray(resourceResult)
                                                ? resourceResult
