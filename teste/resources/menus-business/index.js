...
module.exports = {
	...
	onPost: function (ctx, dpd, loggedUser, menu, callback) {
		try {
			securityBusiness.prepareObjectToPost(ctx, dpd, loggedUser, menu, 'menus', function (objectToPost) {
	...
	onDelete: function (ctx, dpd, menu, oldMenu) {
		try {
			helpers.helperServer.showLog('BierOnStack Debug: Menus -> onDelete');
			auditBusiness.logAction(dpd, helpers.helperServer.both.actions.remove, loggedUser, 'menus', '(' + menu.id + ') - ' + menu.name, ctx.req.headers['x-real-ip'], oldMenu, menu);
		} catch (error) {
