...
var canGet = function (ctx, bbc, loggedUser, permission, callback) {
    try {
        helpers.helperServer.showLog("BierOnStack: Recurso PermissionsBusiness :: Evento :: canGet :: Ação :: Begin");
        securityBusiness.hasPermissionToAction(ctx, bbc, loggedUser, permission, "permissions", helpers.helperServer.both.actions.read, function (canRead) {
...
var canGetByUserPermission = function (ctx, bbc, loggedUser, permission, collectionPermission, callback) {
    helpers.helperServer.showLog("BierOnStack: Recurso PermissionsBusiness :: Evento :: canGetByUserPermission :: Ação :: Begin");
    try {
        securityBusiness.hasPermissionToActionInCollection(ctx, bbc, loggedUser, permission, collectionPermission, function (canRead) {
        ...
        helpers.helperServer.showException(
            helpers.helperServer.both.formatString("BierOnStack: Recurso PermissionsBusiness :: Evento :: canGetByUserPermission :: Exceção :: {0}", error.message)
        );
    }
};
