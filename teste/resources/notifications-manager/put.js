...
try {
	...
	if (!previous.isNotified) {
		helpers.helperServer.showLog(helpers.helperServer.both.formatString('BierOnStack: Recurso NotificationsManaget :: Evento :: put :: Ação :: Notificando {0}', JSON.stringify(notificationObject)));
		...
		dpd.notificationsmanager.del(notificationObject.id);
	}
