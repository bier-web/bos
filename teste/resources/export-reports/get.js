...
try {
    ...
    exportReportsBusiness.canGet(ctx, bbc, me, exportReport, query, function (canGet, error) {
        if (canGet) {
            $addCallback();
            bbc.systemsettings.get({ name: { $in: ["exportPath", "timeZone"] }, $ignoreLimitRecursion: true }, function (systemSettings, error) {
                if (error || systemSettings.length < 2) {
                    helpers.helperServer.showException(
                        helpers.helperServer.both.formatString("BierOnStack: Recurso exportReports :: Evento :: get :: Exceção :: Erro lendo configurações de sistema {0}", error)
                    );
                } else {
                    ...
                    bbc.boswrappergetdata.get(
                        ...
                        function (dataResult, error) {
                            if (error) {
                                helpers.helperServer.showException(
                                    helpers.helperServer.both.formatString("BierOnStack: Recurso exportReports :: Evento :: get :: Exceção :: {0}", error.message)
                                );
                                return;
                            } else {
                                                ...
                                                if (error || exportDefinitions.length == 0) {
                                                    helpers.helperServer.showException(
                                                        helpers.helperServer.both.formatString(
                                                            "BierOnStack: Recurso exportReport :: Evento :: get :: Exceção :: Não existe definição para exportação {0}",
                                                            query.collection.viewName || query.collection.name
                                                        )
                                                    );
                                                } else {
                                                    ...
                                                    if (relatedCollections.length == 0) {
                                                        ...
                                                        helpers.helperServer.exportToExcel(
                                                            exportDefinitions[0],
                                                            dataResult.data,
                                                            exportPath,
                                                            fileName,
                                                                    ...
                                                                    relatedCollections.forEach((rc) => {
                                                                        ...
                                                                        promissesData.push(
                                                                            new Promise(function (resolve) {
                                                                                var settingCollectionMappping = settingKeys.find(
                                                                                    (mappingCollection) => mappingCollection.name === "collectionMappping_" + rc
                                                                                );
                                                                                var collectionName =
                                                                                    typeof settingCollectionMappping != "undefined" ? settingCollectionMappping.value : undefined;
                                                                                collectionName = typeof collectionName != "undefined" ? collectionName : rc;
                                                                                bbc[collectionName].get(
                                                                                    {
                                                                                        id: {
                                                                                ...
                                                                                for (var propertyName in exportDefinition.exportDefinitions) {
                                                                                    ...
                                                                                    if (objectProperty.visible) {
                                                                                        if (propertyName.toLowerCase().indexOf(rc.name) > -1) {
                                                                                            cd[propertyName] =
                                                                                                typeof cd[rc.name] != "undefined"
                                                                                                    ? cd[rc.name][
                                                                                                          propertyName
                                                                                                              .substring(
                                                                                                                  propertyName.toLowerCase().indexOf(rc.name) + rc.name.length,
                                                                                                                  propertyName.length
                                                                                                              )
                                                                                                              .toLowerCase()
                                                                                                      ]
