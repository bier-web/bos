...
navigationTag = function (opts) {
	...
	if (opts.isCreating) {
		...
		dpd.menus.get({ isParent: true, $sort: { order: 1 } }, function (menusParent, error) {
			var _countMenusParent = menusParent.length;
			menusParent.forEach(function (menuParent) {
				dpd.menus.get({ menuParentId: menuParent.id, $sort: { name: 1 } }, function (menusChildren, error) {
					if (error) {
					} else {
						_countMenusParent--;
						...
						menusChildren.forEach(function (menu) {
							if (menu.isSeparator && typeof menusChildren[menusChildren.indexOf(menu) - 1] != 'undefined' && menusChildren[menusChildren.indexOf(menu) - 1].isSeparator) {
