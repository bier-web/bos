...
appSettingTag = function (opts) {
	...
	this.on('mount', function () {
		if (opts.action == helpersWebApp.both().actions.add) {
			app.loading(true);
			dpd.appsettings.post({}, function (newItem) {
				_self.load(newItem);
				app.loading(false);
			});
		}
		...
		if (!opts.onlyBusiness) {
			$('.app-setting-tag .app-setting-form').form({
				...
				onSuccess: function (e) {
					...
					dpd.appsettings.post(
						{
							id: $('.app-setting-tag .app-setting-form #app-setting-id').val(),
							name: $('.app-setting-tag .app-setting-form #app-setting-name').val(),
							sellerEmployeePositionIds: $('.app-setting-tag .app-setting-form #app-setting-seller-employee-position-ids .dropdown').dropdown('get value')[0].split(','),
							...
							saleStatusIndexProgressionIds: $('.app-setting-tag .app-setting-form #app-setting-sale-status-index-progression-ids .dropdown')
								.dropdown('get value')[0]
								.split(','),
							saleStatusNonBlockingIds: $('.app-setting-tag .app-setting-form #app-setting-sale-status-non-blocking-ids .dropdown').dropdown('get value')[0].split(',')
