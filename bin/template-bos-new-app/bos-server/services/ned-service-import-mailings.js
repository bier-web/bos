...
nedServiceImportMailing = function (mailingToImport, serverDeployd) {
	postLead = function (leadToPost, callback) {
		serverDeployd.leads.post(leadToPost, function (newLead, error) {
			if (error) {
	...
	isImportFinished = function (countLeadsImported) {
		if (countLeadsImported == 0) {
			...
			serverDeployd.mailings.post(mailingToImport, function (mailingUpdate, error) {
				if (error) {
				...
				XlsxPopulate.fromFileAsync(mailingToImport.pathURL).then((workbook) => {
					...
					sheetRows.map((row) => {
						...
						if (lead.isReadyToBot) {
							serverDeployd.leads.get({ document: lead.document, $ignoreLimitRecursion: true }, function (duplicatedLead, error) {
								if (error) {
									...
									if (duplicatedLead.length == 0) {
										...
										helpersServer.showException(helpersServer.both.formatString('BierOnStack Debug: nedServiceImportMailings -> Lead duplicado {0}', lead.document));
										_countLeadsToImport = _countLeadsToImport - 1;
										isImportFinished(_countLeadsToImport);
									}
								}
					...
					if (error) {
						...
						setInterval(function () {
							if (!isImportingMailing) {
								...
								serverDeployd.mailings.get({ processingStatusId: { $in: [0, 1] } }, function (mailings, error) {
									helpersServer.showLog(
										helpersServer.both.formatString(
									...
									if (mailings.length > 0) {
										...
										if (mailingsInProgress.length > 0) {
											nedServiceImportMailing(mailingsInProgress[0], serverDeployd);
										} else if (mailingsInProgress.length == 0) {
											nedServiceImportMailing(mailings[0], serverDeployd);
										}
									}
								});
							}
