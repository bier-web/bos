...
try {
	...
	if (!internal) {
		dimensionsBusiness.canGet(ctx, dpd, me, _dimension, function (canGet) {
	...
	if (typeof query.include !== 'undefined' && query.include.indexOf('usergroupdimensions') > -1) {
		dpd.usergroupdimensions.get({ dimensionId: _dimension.id, userGroupId: query.userGroupId, $ignoreLimitRecursion: true }, function (userGroupDimensions, error) {
			if (error) {
