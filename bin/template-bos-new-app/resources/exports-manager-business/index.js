...
var exportsManagerBusiness = {
	canGet: function (ctx, dpd, loggedUser, exportManager, callback) {
		try {
			try {
				helpers.helperServer.showLog('BierOnStack Debug: mediasBusiness_canGet !! Put your custom code to canGet here !!');
				securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, exportManager, 'exportsmanager', helpers.helperServer.both.actions.read, function (canRead) {
					callback(canRead);
				});
			} catch (error) {
	...
	onPost: function (ctx, dpd, loggedUser, exportManager, callback) {
		try {
		} catch (error) {
	...
	onDelete: function (ctx, dpd, loggedUser, exportManager, callback) {
		try {
		} catch (error) {
	...
	prepareObjectToPost: function (ctx, dpd, loggedUser, exportManager, callback) {
		try {
			securityBusiness.prepareObjectToPost(ctx, dpd, loggedUser, exportManager, 'exportsmanager', function (objectToPost) {
