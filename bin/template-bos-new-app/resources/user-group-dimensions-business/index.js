...
var canGet = function (ctx, dpd, loggedUser, userGroupDimension, callback) {
	try {
		helpers.helperServer.showLog('BierOnStack: Recurso UserGroupDimensionsBusiness :: Evento :: canGet :: Ação :: Begin');
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, userGroupDimension, 'usergroups', helpers.helperServer.both.actions.read, function (canRead) {
			callback(canRead);
		});
		helpers.helperServer.showLog('BierOnStack: Recurso UserGroupDimensionsBusiness :: Evento :: canGet :: Ação :: End');
	} catch (error) {
		helpers.helperServer.showException(helpers.helperServer.both.formatString('BierOnStack: Recurso UserGroupDimensionsBusiness :: Evento :: canGet :: Exceção :: {0}', error.message));
	}
};
...
var onPost = function (ctx, dpd, loggedUser, objectItem, callback) {
	try {
		auditBusiness.logAction(dpd, helpers.helperServer.both.actions.add, loggedUser, 'Dimensão no Grupo ', '(' + objectItem.id + ') - ' + objectItem.name, ctx.req.headers['x-real-ip']);
	} catch (error) {
...
var onPut = function (ctx, dpd, loggedUser, userGroupPermission, callback) {
	try {
		securityBusiness.prepareObjectToPost(ctx, dpd, loggedUser, userGroupPermission, 'usergroups', function (userGroupDimensionToPost) {
	...
	try {
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, userGroupPermission, 'usergroups', helpers.helperServer.both.actions.edit, function (canEdit) {
			if (canEdit) {
				...
				auditBusiness.logAction(dpd, helpers.helperServer.both.actions.edit, loggedUser, 'Usuário na Dimensão ', '(' + userGroupPermission.id + ') - Usuário: ' + userGroupPermission.userId + '  - Dimensão: ' + userGroupPermission.dimensionId, ctx.req.headers['x-real-ip']);
			}
...
var onDelete = function (ctx, dpd, loggedUser, objectItem, callback) {
	try {
		auditBusiness.logAction(dpd, helpers.helperServer.both.actions.remove, loggedUser, 'Dimensão no Grupo ', '(' + objectItem.id + ') - ' + objectItem.name, ctx.req.headers['x-real-ip']);
	} catch (error) {
