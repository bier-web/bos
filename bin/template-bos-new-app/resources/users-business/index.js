...
module.exports = {
	canLogin: function (ctx, dpd, user, callback) {
		try {
			helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: canLogin :: Ação :: Begin');
			auditBusiness.logAction(dpd, helpers.helperServer.both.actions.tryLogin, ctx.body, 'Usuários', ctx.body.username + ' *** Agent: ' + ctx.req.headers['user-agent'], ctx.req.headers['x-real-ip']);
			var _error = 0;
	...
	onLogin: function (ctx, dpd, loggedUser, callback) {
		try {
			helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Ação :: Begin');
			auditBusiness.logAction(dpd, helpers.helperServer.both.actions.login, loggedUser, 'Usuários', '(' + loggedUser.id + ') - ' + loggedUser.username, ctx.req.headers['x-real-ip']);
			...
			dpd.usergroupusers.get({ userId: loggedUser.id }, function (userGroupUsers, error) {
				if (error) {
					helpers.helperServer.showException(helpers.helperServer.both.formatString('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Exceção :: {0}', error.message));
				} else {
						...
						userGroupUsers.forEach((userGroupUser) => {
							dpd.usergroups.get(
								...
								function (userGroup, error) {
									if (error) {
										helpers.helperServer.showException(helpers.helperServer.both.formatString('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: !! Usuário vinculado a um grupo inexistente ou inativo!! Exceção :: {0}', error.message));
									} else {
										dpd.usergroupmenus.get(
											{
												userGroupId: userGroup.id
											},
											function (userGroupMenus) {
												if (userGroupMenus.length == 0) {
													helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Ação :: Usuário vinculado a grupo sem menus!');
												} else {
													helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Ação :: Menus permitidos vinculado ao usuário!');
												}
												loggedUser.userGroupMenus = userGroupMenus;
												dpd.usergrouppermissions.get(
													{
														userGroupId: userGroup.id,
														include: 'permission'
													},
													function (userGroupPermissions, error) {
														if (error) {
															...
															if (userGroupPermissions.length == 0) {
																helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Ação :: Usuário vinculado a grupo sem permissões!');
															} else {
																helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Ação :: Permissões permitidas vinculadas ao usuário!');
															}
															...
															dpd.usergroupdimensions.get(
																...
																function (userGroupDimensions) {
																	if (userGroupDimensions.length == 0) {
																		helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Ação :: Usuário vinculado a grupo sem dimensões!');
																	} else {
																		helpers.helperServer.showLog('BierOnStack: Recurso UsersBusiness :: Evento :: onLogin :: Ação :: Dimensões permitidas vinculadas ao usuário!');
																	}
	...
	canDelete: function (ctx, dpd, loggedUser, user, callback) {
		try {
			helpers.helperServer.showLog('BierOnStack Debug: usersBusiness -> canDelete ');
			securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, user, 'users', helpers.helperServer.both.actions.remove, function (canDelete) {
				if (canDelete) {
					helpers.helperServer.showLog(helpers.helperServer.both.formatString('BierOnStack Debug: usersBusiness -> canDelete -> Permission allowed, deleting {0}', user.username));
				}
	...
	onDelete: function (ctx, dpd, loggedUser, user) {
		try {
		...
		auditBusiness.logAction(dpd, helpers.helperServer.both.actions.edit, user, 'Usuários', '(' + user.id + ') - ' + user.username, ctx.req.headers['x-real-ip']);
	},
	prepareObjectToPost: function (ctx, dpd, loggedUser, user, callback) {
		try {
			securityBusiness.prepareObjectToPost(ctx, dpd, loggedUser, user, 'users', function (userToPost) {
	...
	canGet: function (ctx, dpd, loggedUser, user, callback) {
		try {
			if (user.id === loggedUser.id) {
				callback(true);
			} else {
				securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, user, 'users', helpers.helperServer.both.actions.read, function (canRead) {
					callback(canRead);
				});
			}
		} catch (error) {
	...
	canPost: function (ctx, dpd, loggedUser, user, callback) {
		try {
			...
			switch (_error) {
				case 0:
					...
					auditBusiness.logAction(dpd, helpers.helperServer.both.actions.add, user, 'Usuários', '(' + user.id + ') - ' + user.username, ctx.req.headers['x-real-ip']);
					callback(true);
					break;
	...
	canPut: function (ctx, dpd, loggedUser, user, callback) {
		try {
			...
			if (user.id === loggedUser.id) {
				...
				securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, user, 'users', helpers.helperServer.both.actions.edit, function (canEdit) {
					if (canEdit) {
						helpers.log('UsersBusiness -> canPut -> Registrando log de auditoria');
						auditBusiness.logAction(dpd, helpers.helperServer.both.actions.edit, user, 'Usuários', '(' + user.id + ') - ' + user.username, ctx.req.headers['x-real-ip']);
					}
	...
	onPost: function (ctx, dpd, loggedUser, user, callback) {
		//NewFeature
		try {
				...
				dpd.students.post(
					...
					function (student, error) {
						if (error) {
							...
							dpd.appsettings.get(function (appSettings, error) {
								if (error) {
	...
	onPut: function (ctx, dpd, loggedUser, user, callback) {
		try {
			securityBusiness.prepareObjectToPost(ctx, dpd, loggedUser, user, 'users', function (_userToPost) {
