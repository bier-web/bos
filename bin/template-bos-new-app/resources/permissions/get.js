...
try {
	...
	if (!internal) {
		if (typeof query.userPermissionCollection !== 'undefined') {
			permissionsBusiness.canGetByUserPermission(ctx, dpd, me, _permission, query.userPermissionCollection, function (canGet) {
			...
			permissionsBusiness.canGet(ctx, dpd, me, _permission, function (canGet) {
	...
	if (typeof query.include !== 'undefined' && query.include.indexOf('usergrouppermissions') > -1) {
		dpd.usergrouppermissions.get({ permissionId: _permission.id, userGroupId: query.userGroupId, $ignoreLimitRecursion: true }, function (userGroupPermissions, error) {
			if (error) {
