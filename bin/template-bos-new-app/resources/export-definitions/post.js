try {
	...
	exportDefinitionsBusiness.prepareObjectToPost(ctx, dpd, me, exportDefinition, function (objectItemToPost) {
		helpers.log('exportdefinitions -> onPost -> Chamando exportDefinitionsBusiness -> canPost');
		exportDefinitionsBusiness.canPost(ctx, dpd, me, objectItemToPost, function (canPost, message) {
			if (canPost) {
				helpers.log('exportdefinitions -> onPost -> canPost ok, chamando onPost');
				exportDefinitionsBusiness.onPost(ctx, dpd, me, objectItemToPost);
			} else {
