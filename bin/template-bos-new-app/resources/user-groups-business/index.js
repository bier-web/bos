...
var canGet = function (ctx, dpd, loggedUser, userGroup, callback) {
	try {
		helpers.helperServer.showLog('BierOnStack: Recurso UserGroupsBusiness :: Evento :: canGet :: Ação :: Begin');
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, userGroup, 'usergroups', helpers.helperServer.both.actions.read, function (canRead) {
...
var canPost = function (ctx, dpd, loggedUser, userGroup, callback) {
	try {
		helpers.helperServer.showLog('BierOnStack: Recurso UserGroupsBusiness :: Evento :: post :: Ação :: Begin');
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, userGroup, 'usergroups', helpers.helperServer.both.actions.add, function (canAdd) {
			if (canAdd) {
				helpers.helperServer.showLog('BierOnStack: Recurso UserGroupsBusiness :: Evento :: post :: Ação :: End usuário autorizado');
				auditBusiness.logAction(dpd, helpers.helperServer.both.actions.add, loggedUser, 'Grupo de Usuário ', '(' + userGroup.id + ') - ' + userGroup.name, ctx.req.headers['x-real-ip']);
			}
...
var canPut = function (ctx, dpd, loggedUser, userGroup, callback) {
	try {
		helpers.log('userGroups -> canPut -> begin');

		helpers.log('userGroups -> canPut -> Chamando securityBusiness -> hasPermissionToAction');
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, userGroup, 'usergroups', helpers.helperServer.both.actions.edit, function (canEdit) {
			if (canEdit) {
				helpers.log('userGroups -> canPut -> Registrando log de auditoria');
				auditBusiness.logAction(dpd, helpers.helperServer.both.actions.edit, loggedUser, 'Grupo de Usuário ', '(' + userGroup.id + ') - ' + userGroup.name, ctx.req.headers['x-real-ip']);
			}
...
var canDelete = function (ctx, dpd, loggedUser, userGroup, callback) {
	try {
		helpers.log('userGroups -> canDelete -> begin');

		helpers.log('userGroups -> canDelete -> Chamando securityBusiness -> hasPermissionToAction');
		securityBusiness.hasPermissionToAction(ctx, dpd, loggedUser, userGroup, 'usergroups', helpers.helperServer.both.actions.remove, function (canDelete) {
			if (canDelete) {
				var _error = 0;

				helpers.log('UserGroupsBusiness -> canDelete -> Verificando se existem usuários vinculados ao grupo');
				dpd.usergroupusers.get({ userGroupId: userGroup.id, isAllowed: true }, function (userGroupUsers, error) {
					if (error) {
...
var onDelete = function (ctx, dpd, loggedUser, userGroup, callback) {
	try {
		helpers.log('UserGroupsBusiness -> onDelete -> Apagando vínculo com permissões');
		dpd.usergrouppermissions.get({ userGroupId: userGroup.id }, function (userGroupPermissions, error) {
			userGroupPermissions.forEach((userGroupPermission) => {
				dpd.usergrouppermissions.del(userGroupPermission.id, function (userGroupPermission, error) {
					if (error) {
		...
		dpd.usergroupmenus.get({ userGroupId: userGroup.id }, function (userGroupMenus, error) {
			userGroupMenus.forEach((userGroupMenu) => {
				dpd.usergroupmenus.del(userGroupMenu.id, function (userGroupMenu, error) {
					if (error) {
		...
		dpd.usergroupdimensions.get({ userGroupId: userGroup.id }, function (userGroupDimensions, error) {
			userGroupDimensions.forEach((userGroupDimension) => {
				dpd.usergroupdimensions.del(userGroupDimension.id, function (userGroupDimension, error) {
					if (error) {
		...
		dpd.usergroupusers.get({ userGroupId: userGroup.id }, function (userGroupUsers, error) {
			userGroupUsers.forEach((userGroupUser) => {
				dpd.usergroupusers.del(userGroupUser.id, function (userGroupUser, error) {
					if (error) {
		...
		auditBusiness.logAction(dpd, helpers.helperServer.both.actions.remove, loggedUser, 'Grupo de Usuário ', '(' + userGroup.id + ') - ' + userGroup.name, ctx.req.headers['x-real-ip']);
	} catch (error) {
...
var onPost = function (ctx, dpd, loggedUser, userGroup, callback) {
	try {
